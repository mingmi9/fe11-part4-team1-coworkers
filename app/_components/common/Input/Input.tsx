...
const Input: React.FC<InputProps> = ({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (type === 'password') {
      ...
      } else if (newValue.length < 8 || !passwordPattern.test(newValue)) {
        setError('비밀번호가 틀립니다.');
      } else {
        setError(null);
      }
    ...
    <div className="flex flex-col mb-4">
      ...
      <div className="relative">
        <input
          ...
          className={`
            text-text-primary border-border-primary bg-background-secondary rounded-xl px-2.5
            focus:outline-none  focus:ring-2 focus:ring-interaction-hover placeholder:text-text-default 
            ${error ? 'border-status-danger' : 'border-border-primary' }
            w-[21.44rem] h-[2.8125rem] text-[1rem]
            mobile:w-[21.44rem] mobile:h-[2.8125rem] mobile:text-[1rem]
            tablet:w-[21.44rem] tablet:h-[2.75rem] tablet:text-[1rem]
            pc:w-[28.75rem] pc:h-[3rem] pc:text-[1.125rem]
          `}
        />
        {type === 'password' && (
          <span
